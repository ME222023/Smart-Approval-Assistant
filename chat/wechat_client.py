...
class WechatClient:
    ...
    该类用于与企业微信 API 进行交互，包括获取 access_token、发送消息、上传文件等功能。

    属性:
    corpid (str): 企业微信的企业ID。
    app (tuple): 包含应用的 AgentId 和 Secret 的元组。
    """
    ...
    def __init__(self):  # app_id 按app顺序编号
        ...
        初始化WechatClient类实例
        """
        self.agentid, self.corpsecret = WechatClient.app
    ...
    def get_access_token(self) -> str:
        """
        获取企业微信的访问令牌（access_token）。
        @return: 返回字符串类型的 access_token。
        """
        if self.access_token and self.access_token["expires"] - 60 > int(time.time()):
            ...
            response = requests.get(
                "https://wxw-sit.axzq.com.cn/cgi-bin/gettoken?corpid={corpid}&corpsecret={corpsecret}".format(
                    corpid=WechatClient.corpid, corpsecret=self.corpsecret))
    ...
    def send_text(self, _message: str, uid_list: List[str]=['name1|name2']) -> bool:
        """
        发送文本消息到指定的用户列表。
        @param _message: 要发送的文本消息内容，支持换行和 HTML A 标签。
        @param uid_list: 用户 ID 列表，默认值为 ['name1|name2']，用户 ID 在企业微信通讯录中可以找到。
        @return: 返回布尔值，表示消息是否成功发送。
        """
        uid = "|".join(uid_list)  # userid 在企业微信-通讯录-成员-账号
        json_dict = {
            "touser": uid,
            "msgtype": "text",
        ...
        logger.debug("urls https://wxw-sit.axzq.com.cn/cgi-bin/message/send")
        ...
        logger.debug("send to " + uid + ' ' + json.loads(response_send.text)['errmsg'])
        logger.debug(response_send.text)
        return json.loads(response_send.text)['errmsg'] == 'ok'
    
    # --------------------------------本项目中，以下函数暂时没有用到-----------------------------------------
    # 上传临时文件素材接口,图片也可使用此接口,20M上限, 该media_id仅3天内有效
    def post_file(self, filepath: str, filename: str) -> str:
        """
        上传临时文件素材接口, 图片也可使用此接口, 20M上限, 该 media_id 仅3天内有效。

        @param filepath: 文件路径。
        @param filename: 文件名。
        @return: 返回上传成功后的 media_id，失败则返回 None。
        """
    ...
    def send_img(self, _message, useridlist=['name1|name2']) -> bool:
        """
        向应用发送图片接口, _message 为上传临时素材后返回的 media_id。

        @param _message: 上传临时素材后返回的 media_id。
        @param useridlist: 用户 ID 列表，默认值为 ['name1|name2']，用户 ID 在企业微信通讯录中可以找到。
        @return: 返回布尔值，表示消息是否成功发送。
        """
    ...
    def send_file(self, _message, useridlist=['name1|name2']) -> bool:
    ...
    def download_file(self, media_id):
        ...
        logging.info(f"download file: {res.status_code}, {res.headers}")
...
# 回复“审批请求确认task_id” 指定请求开始分析""", ['17a755177f4ad1f1d99bf8042cfbb74a'])
    ...
    # print(qy.get_oa_account("17a755177f4ad1f1d99bf8042cfbb74a"))
