...
def generate_output(cus_status):
    output = {}

    for cus_id, tasks in cus_status.items():
        messages = []
        for task_id, state in tasks.items():
            messages.append(f"{task_id}{state}")
        output[cus_id] = "，".join(messages)
...
# }
    ...
    # wait_tasks = [key.decode("utf-8").split('_')[-1] for key in client.keys('wait_*')]
    # print(wait_tasks)        
    client.rpush(f"wait_{'123'}", 'a')
    client.rpush(f"wait_{'124'}", 'a')  
    client.rpush(f"wait_{'125'}", 'b')   
    dict_ = trial(True) 
    print(dict_) 
