...
# # rule1: 告警时间需晚于生效时间
def rule1(info:dict = None, cus_id: str = None) -> str:
    """
    返回"任务负责人至少2个"的判断字符串
    """
...
#     """
#     返回"请求任务审批者需是任务开发者或任务负责人"的判断字符串
#     """
#     in_charge = info.get("inCharge")
...
def getFuncNames(filename) -> list:
    """
    返回rules.py文件中函数名称列表
    """
...
def checkRules(info:scheduleInfo, cus_id: str = None) -> list:
    ...
    functions = getFuncNames(filename)
    
    # 排除 'checkRules' 函数自身
    functions = [f for f in functions if f != 'checkRules' and f != 'get_function_names']
...
if __name__ == "__main__":
    ...
    # data_trial = getLatestTaskInfo("10037289")
    # print(data_trial)
