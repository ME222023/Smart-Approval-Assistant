...
def generate_messages(instruction: str):
    """生成请求的chats信息，匹配promot,func_des"""
    prompt = "作为一个很棒的调度系统审核AI助理, 你的任务是帮助用户审核调度任务，引导用户提供调度的信息包括调度任务的id，回答用户相关的问题"
    user_message = {
        "role": "user",
        "content": instruction
    }
...
def repsone_gpt(chat_info):
    message = generate_messages(chat_info)
