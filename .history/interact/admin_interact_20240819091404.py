...
def getTaskUserIdDict() -> dict:
    ...
    # Get all values from the keys that match the pattern
    for key in wait_keys:
        task_id = key.decode('utf-8').split('_')[1]  # Extract task_id from the key
        with RedisClient(redis_url) as client:
            byte_values = client.lrange(key, 0, -1)  # Get all values from the list
        str_values = [value.decode('utf-8') for value in byte_values]
        
        # Assuming each key has a single cus_id associated with it
        if str_values:
...
def getTaskIdsByUid(reversed_dict: dict, u_name: str) -> list:
...
# "通过"或者"不通过"名下的所有任务
def updateTaskStatus(task_dict: dict, user_name: str, status: str) -> dict:
    # 初始化一个新的字典用于存储更新后的任务状态
    updated_dict = {}
    combine_str = ""
    if user_name in cusName2cusId:
        user_id = cusName2cusId[user_name]
        # 遍历输入字典
...
def printAprroval(cus_status: dict = {}):
    ...
    for cus_id, tasks in cus_status.items():
        ...
        # 消息不为空且cus_id不是管理员的  
        if messages and (cus_id not in admin_ids):
...
# 查看名下所有任务
def checkTaskIdByName(names: list, dict: dict) -> str:
    message = ""
    for name in names: 
...
def checkAdminMsg(task_id, msg_list: list, cus_id) -> str:
    msg = ""
    if all(item is None for item in msg_list):
        msg += f"任务{task_id}通过规则检测。"
        if cus_id in admin_ids
        with RedisClient(redis_url) as client: 
        ...
        try:
            ...
            if category == "view":
                ...
                if not tasks_id or len(tasks_id) >= 10:
                    # 查看名下所有任务
                    if names:
                        message += checkTaskIdByName(names, dict)
                    else:
                        message += viewTopk(3) 
                # 查看具体id的任务
                else:
            ...
            # 通过待审批任务
                ...
                if not tasks_id:
                    # if names:
                    for name in names:
                        cus_status = updateTaskStatus(dict,name,"通过")
                    # else:
                    #     tasks_id = getTasksID()
                    #     tasks_id = [key.decode("utf-8").split('_')[-1] for key in client.keys('waitt_*')]
                else:
            ...
            # 不通过待审批任务
                ...
                if not tasks_id:
                    # if names:
                    for name in names:
                        cus_status = updateTaskStatus(dict,name,"不通过")
                    # else:
                    #     tasks_id = getTasksID()
                    #     tasks_id = [key.decode("utf-8").split('_')[-1] for key in client.keys('waitt_*')]
                else:
                ...
                if not tasks_id:
                    qw.send_text("请输入需要检查的任务ID。", [admin_id])
                else:
                    # 待审批队列当中的id
                    combined_msgs = []
                    for t_id in tasks_id:
                        # 待审批队列id列表
                        task_list = getTasksID()
                        # 若id在待审批队列当中
                        if t_id in task_list:
                            task_user_id_dict = getTaskUserIdDict()
                            cus_id = task_user_id_dict[t_id]
                        else:
                            cus_id = ""
                        return_info = getLatestTaskInfo(t_id)
                        ...
                        if not cus_id:
                            cus_id = admin_id
                        return_msg = checkAdminMsg(t_id, msg_list, cus_id)
