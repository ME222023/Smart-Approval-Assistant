...
def getReverseDict() -> dict:
    ...
    for key in keys:
        ...
        for value in values:
            ...
            reversed_kv_pairs[value].append(key.decode('utf-8'))
...
def getTaskIdsByUid(reversed_dict, u_name) -> list:
    if u_name in cusName2cusId:
        ...
        # 去掉 'waitt_' 前缀并返回任务ID列表
            return [task_id.replace('waitt_', '') for task_id in reversed_dict[u_id]]
        else:
                ...
                if not tasks_id or len(tasks_id) >= 10:
                    ...
                    if names:
                        ...
                        for name in names: 
                            ...
                            message += f"{name}名下的任务如下：\n"
                            for i in range(0, len(task_list), 4):
                                # 获取当前行的任务列表
                                current_line_tasks = task_list[i:i+4]
                                # 将当前行的任务用空格连接，并添加到结果字符串中
                                formatted_message += " ".join(current_line_tasks) + "\n"
                                message += "\n".join(task_list)
                    else:
                        message += viewTopk(3) 
                # 查看具体id的任务
