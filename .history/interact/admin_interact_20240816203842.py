...
# "通过"或者"不通过"名下的所有任务
# def updateTaskStatus(dict: dict, u_name: str, status: str) -> dict:
#     if u_name in cusName2cusId:
#         u_id = cusName2cusId[u_name]
#         if u_id in dict:
#             task_list = dict[u_id]
...
def updateTaskStatus(task_dict: dict, cusName2cusId: dict, u_name: str, status: str) -> dict:
    if u_name in cusName2cusId:
        u_id = cusName2cusId[u_name]
        if u_id in task_dict:
            task_list = task_dict[u_id]
            updated_tasks = [{task_id: status} for task_id in task_list]
            task_dict[u_id] = updated_tasks
        else:
            print(f"u_id '{u_id}' does not exist in the dictionary.")
    else:
        print(f"u_name '{u_name}' does not exist in the cusName2cusId mapping.")
