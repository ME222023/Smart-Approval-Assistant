    ...
    for k in keys:
        ...
        try:
            ...
            if isinstance(answer, dict):
                ...
                qw.send_text("\n\n".join(combined_msgs) + "\n\n请解决以上的问题。", ["1730846d950e0a5709ac2ea44138df09"])
        except json.JSONDecodeError as e:
            logger.error(f"JSONDecodeError: {e}")
            qw.send_text(data, ["1730846d950e0a5709ac2ea44138df09"])
        except PermissionError as e:
            logger.error(f"PermissionError: {e}")
            qw.send_text("用户没有权限，已通知管理员进行处理。", ["1730846d950e0a5709ac2ea44138df09"])
        except MemoryError as e:
            logger.critical(f"MemoryError: {e}")
            qw.send_text("发生了内存错误，已通知管理员进行处理。", ["17a755177f4ad1f1d99bf8042cfbb74a"])
        except SystemExit as e:
            logger.critical(f"SystemExit: {e}")
            qw.send_text("发生了系统退出，已通知管理员进行处理。", ["17a755177f4ad1f1d99bf8042cfbb74a"])
        except KeyboardInterrupt as e:
            logger.critical(f"KeyboardInterrupt: {e}")
            qw.send_text("发生了键盘中断，已通知管理员进行处理。", ["17a755177f4ad1f1d99bf8042cfbb74a"])
        except Exception as e:
            logger.critical(f"Unexpected error: {e}")
            qw.send_text("系统出现异常，已通知管理员进行处理。", ["17a755177f4ad1f1d99bf8042cfbb74a"])
    time.sleep(2)
