...
def waitApproval(tasks_id: list,  admin_id: str, wait_approve = True) -> dict:
    # wait_tasks = getWait()
    wait_tasks = [key.decode("utf-8").split('__')[-1] for key in client.keys('wait_*')]
    # print(wait_tasks)
    # wait_re = reverse()
    status = {}
    temp = {}
    ...
    for id in tasks_id:
        if id not in wait_tasks:
            ...
            # print(cus_id)
            # 获取与 task_id 关联的所有 cus_id
            cus_ids = client.lrange(f"waitt__{id}", 0, -1)
            # 将字节类型转换为字符串
            cus_ids = [cus_id.decode('utf-8') for cus_id in cus_ids]
            if wait_approve:
                status[id] = "通过"
    ...
    def adminApprove(admin_id: str, msg: str):
        ...
        try:
            ...
            if "查看" in msg:
                ...
                # tasks_id = getWait()
                tasks_id = [key.decode("utf-8").split('__')[-1] for key in client.keys('wait_*')]
