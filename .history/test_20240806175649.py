...
def waitApproval(tasks_id: list[str], wait_approve = True):
    wait_tasks = getWait()
    wait_re = reverse()
    # keys = r_.keys("wait_*")
    for id in tasks_id:
...
# r_.rpush(f"wait_{cus_id}","123")
# r_.rpush(f"wait_{cus_id}","124")
# r_.rpush(f"wait_{cus_id}","125")
# r_.rpush(f"wait_{cus_id}","126")
