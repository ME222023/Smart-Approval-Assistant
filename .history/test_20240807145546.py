...
def waitApproval(tasks_id: list,  admin_id: str, wait_approve = True):
    ...
    for id in tasks_id:
        if id not in wait_tasks:
            # 发送消息给admin：该任务id不存在
            combine_str2 += f"任务{id}不存在，请查看ID输入是否正确。\n"
            ...
            if wait_approve:
                ...
                if msg == "操作成功！":
                    ...
                    combine_str2 += f"已完成任务{id}的审批。\n"
                else:
                    combine_str2 += f"已完成任务{id}的审批，请不要重复操作。\n"
                combine_str1 += f"任务{id}已审批通过，请注意查看。\n"
            # 批准不通过
            else:
                ...
                if msg == "操作成功！":
                    ...
                    combine_str2 += f"已完成任务{id}的审批，请不要重复操作。\n"
                combine_str1 += f"任务{id}审批不通过，请修改后再做尝试。\n"
            qw.send_text(combine_str1, [cus_id])
    qw.send_text(combine_str2, [admin_id])
...
def adminApprove(admin_id: str, msg: str):
    ...
    try:
        ...
        if "查看" in msg:
            ...
            tasks_id = getWait()
            waitApproval(admin_idtasks_id, False)
        elif "不通过" in msg:
            waitApproval(tasks_id, False)
        elif msg == "审批通过":
            tasks_id = getWait()
    ...
    qw.send_text("系统出现异常，已通知管理员进行处理。", [admin_id])
time.sleep(2)
