...
def waitApproval(tasks_id: list = [], admin_id: str = None, wait_approve = True) -> dict:
    """
    得到的字典结构:
    ...
    if not wait_tasks:
        qw.send_text(f"待审批任务列表为空，暂时没有可以审批的任务。\n", [admin_id])
        return
    print("----------------------------------------")
    print(wait_tasks)
    # wait_re = reverse()
    ...
    def adminApprove(admin_id: str, msg: str):
        ...
        try:
            ...
            if cateogry == "查看":
                ...
                qw.send_text("除了输入任务ID，请输入具体操作：审批通过/审批不通过/查看任务，再加上具体任务ID，尝试一下。\n", [admin_id])
