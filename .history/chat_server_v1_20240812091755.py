...
def returnWait(task_id: str, msg: list) -> str:
    """
    检查指定任务是否在等待任务列表中，并根据消息内容返回相应的提示信息。

    @param task_id: 任务的唯一标识符，用于在等待任务列表中进行查找。
    @param msg: 消息列表，包含与任务相关的信息。如果列表中的所有项均为 None，且任务不在等待任务列表中，则返回特定提示信息。

    @return: 如果消息列表中的所有项均为 None 且任务不在等待任务列表中，则返回提示任务已通过规则检测并需要审批的字符串；否则返回空字符串。
    """
    logger.debug(f"检查任务ID: {task_id}")
    
    wait_tasks = [key.decode("utf-8").split('_')[-1] for key in client.keys('waitt_*')]
    logger.debug(f"等待任务列表: {wait_tasks}")
    
    if all(item is None for item in msg):
        logger.debug(f"消息列表中的所有项均为 None: {msg}")
    else:
        logger.debug(f"消息列表内容: {msg}")
    
    if task_id in wait_tasks:
        logger.debug(f"任务 {task_id} 在等待任务列表中")
    else:
        logger.debug(f"任务 {task_id} 不在等待任务列表中")
    
    output = ""
    if all(item is None for item in msg) and task_id not in wait_tasks:
        output += f"任务{task_id}已通过规则检测，请进行审批。"
        logger.info(f"任务 {task_id} 已通过规则检测，请进行审批")
    return output
...
def checkId(tasks_id: list, cus_id: str) -> list:
    ...
    for id in tasks_id:
        ...
        if not judge_id:
            ...
            msg_list = ["没有查询到相关的任务。\n请检查输入的任务ID是否正确和该任务是否有用户申请审批。"]
        else:
            msg_list = checkRules(return_info, cus_id)
        return_msg_ad = returnWait(id, msg_list)
        return_msg_cus = returnMsg(id, msg_list, cus_id)
