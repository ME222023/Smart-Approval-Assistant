...
def checkTopk(k: int) -> str:
    ...
    if not tasks_id:
        ...
        for task_id in top_k_tasks_id:
            ...
            output += "\n\n"
        output += f"以上只显示了待审批队列中前{k}个任务的信息。"
    return output
...
def waitApproval(tasks_id: list, admin_idwait_approve = True) -> dict:
    # wait_tasks = getWait()
    ...
    def adminApprove(admin_id: str, msg: str):
        ...
        try:
            ...
            if cateogry == "查看":
                ...
                if not tasks_id:
                    tasks_id = [key.decode("utf-8").split('_')[-1] for key in client.keys('waitt_*')]
                cus_status = waitApproval(tasks_id, True)
                print(cus_status)
                generate_output(cus_status)
            elif cateogry == "不通过":
                if not tasks_id:
                    tasks_id = [key.decode("utf-8").split('_')[-1] for key in client.keys('waitt_*')]
                cus_status = waitApproval(tasks_id, False)
                print(cus_status)
                generate_output(cus_status)
