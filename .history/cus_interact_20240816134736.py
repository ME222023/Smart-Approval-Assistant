...
from scheduler.schedule_handle import getLatestTaskInfo
...
admin_id = config.config['server']['admin_ids']
...
def cusInteract(k: str, cus_id: str):
    ...
    try:
        ...
        if combined_ad:
            # 发送合并后的消息给管理员
            qw.send_text(combined_ad, admin_id)
        qw.send_text("\n\n".join(combined_msgs), [cus_id])  
    except json.JSONDecodeError as e:
        logger.error(f"JSON decode error: {e}")
        qw.send_text(answer, [cus_id])
        ...
        logger.error(f"PermissionError: {e}")
        qw.send_text("没有权限，已通知管理员进行处理。", [cus_id])
        qw.send_text(f"用户{cus_id}没有权限，请管理员进行处理。", admin_id)
    except MemoryError as e:
        logger.error(f"MemoryError: {e}")
        qw.send_text("发生了内存错误，已通知管理员进行处理。", [cus_id])
        qw.send_text("发生了内存错误，请管理员进行处理。", admin_id)
    except SystemExit as e:
        logger.error(f"SystemExit: {e}")
        qw.send_text("发生了系统退出，已通知管理员进行处理。", [cus_id])
        qw.send_text("发生了系统退出，请管理员进行处理。", admin_id)
    except LoginException as e:
        logger.error(f"LoginException: {e}")
        qw.send_text("登录失败，请重新登录授权。", admin_id)
        qw.send_text("登录失败，已通知管理员进行处理。", [cus_id])
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        qw.send_text("系统出现异常，已通知管理员进行处理。", [cus_id])
        qw.send_text("系统出现异常，请管理员进行处理。", admin_id)
time.sleep(2)
