...
def returnWait(task_id: str = None, msg: list = None) -> str:
    # print(task_id)
    """
    返回
    @param task_id: 任务ID
    @param msg: 规则匹配后的消息队列
    @return: 生成请求审批后用户的返回信息
    """
...
def returnMsg(task_id: str = None, msg: list = None, cus_id: str = None) -> str:
    # print("---------------------------------------")
...
def checkId(tasks_id: list = None, cus_id: str = None) -> list:
    combined_msgs = []
    combined_ad = []
    for id in tasks_id:
...
def getData(k: bytes = None) -> str:
    """
    根据提供的字节数据返回相应的字符串。如果未提供字节数据，则使用空字节。

    @param k: 字节数据，用于生成返回的字符串。如果未提供，则默认为空字节。
    @return: 返回生成的字符串。
    """
...
#             time.sleep(2)
with RedisClient(redis_url="redis://:123@10.5.5.73:16379/v1") as client:
    def regular(k: str = None, cus_id: str = None):
        answer = getData(k)
        try:
            result = json.loads(answer)
            tasks_id = result.get('tasks_id')
