...
import json
import logging
from LLM.assistant import getType
...
def printAprroval(cus_status: dict = {}):
    ...
    for cus_id, tasks in cus_status.items():
        messages = []
        logger.debug(f"cus_id: {cus_id}, tasks: {tasks}")
        for task_id, state in tasks.items():
            logger.debug(f"task_id: {task_id}, state: {state}")
            if state == "通过":
                logger.debug(f"任务{task_id}已审批通过，请注意查看。")
...
def adminInteract(admin_id: str, msg: str):
    ...
    try:
        ...
        logger.debug(f"解析消息结果: tasks_id={tasks_id}, category={category}")
        # print(tasks_id)
        # print(cateogry)
        # tasks_id = list(map(str, tasks_id))
        # print(type(tasks_id))
