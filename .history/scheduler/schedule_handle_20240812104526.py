...
# 认证token
TOKEN = "eyJhbGciOiJIUzUxMiJ9..CsGxcmh4eWhUv_P9l0WS7Td9xWbPpCJeRGI-LZN9qKWgmpxAjiMhj966s9cYGXcs8KqESZ4zpeGSo8g71M1heg"
...
def task_info1(t_id: str = None) -> dict:
...
def approval(task_id: str, approval = True):
    ...
    @param task_id: 申请者的msg_{ID}键值
    @param approval: 审批通过与否的判断，默认值为True即通过，False即不通过
    @return: 返回LLM回复的字符串。
    """
...
def by_user(user: str = None, approval = True):
    if user != "":
        print("通过开发者信息审批")
...
def by_task_id(task_id: str = None) -> str:
        tt = task_list(task_id=task_id)
        if len(tt) > 0:
            for t in tt:  
                t_id = t.get("id")
